...
function showScore(result) {
  ...
  $('scale-fill').style.width = `${(score.iq-70)/.6}%`;
  $('scale-marker').style.left = `${(score.iq-70)/.6}%`; $('scale-marker').hidden = false;
  $('score-note').textContent = score.calibrated
    ? `${(result.elapsed_ms / 1000).toFixed(2)}秒 · 順位による再尺度化 · 元の代理値 ${score.original_iq.toFixed(2)}${score.outside_reference ? ' · 基準分布外のため端点に固定' : ''}`
    : `${(result.elapsed_ms / 1000).toFixed(2)}秒${score.clipped ? ' · 表示範囲外のためクリップしています。raw IQ代理値: ' + score.raw_iq.toFixed(2) : ''}`;
}
function detail(span) {
  const message = current.score.calibrated
    ? `「${span.text}」を空白に置換: 再尺度化スコア ${current.score.iq.toFixed(2)} → ${span.without_iq.toFixed(2)}。差分 ${signed(span.delta)}。${span.empty_variant ? '除去後は空文です（学習分布外の可能性）。' : ''}`
    : `「${span.text}」を空白に置換: raw代理値 ${current.score.raw_iq.toFixed(2)} → ${span.without_raw_iq.toFixed(2)}。元の値への差分 ${signed(span.delta)}。${span.empty_variant ? '除去後は空文です（学習分布外の可能性）。' : ''}`;
...
function showHeatmap(result) {
  ...
  for (const part of [...result.spans].sort((a, b) => Math.abs(b.delta) - Math.abs(a.delta)).slice(0, 12)) {
    ...
    const value = document.createElement('span'); value.textContent = signed(part.delta) + (result.score.calibrated ? ' 再尺度化点' : ' raw代理点');
...
$('export').addEventListener('click', () => {
  ...
  const report = {format: 'text-iq-result-v1', created_at: new Date().toISOString(), model_id: manifest.id,
    ...
    warning: '文章の相対順位を70〜130へ再尺度化した代理指標。人間のIQではなく精度改善でもない。除去差分は再尺度化した表示点の差であり、教師の思考過程/因果的根拠/加法的寄与ではない。'};
