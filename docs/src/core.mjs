import {validateNormalModel, scoreNormalModel} from './normal-score.mjs';
...
export function validateManifest(m) {
  ...
  if (![1, 2].includes(m.schema_version) || !/^[0-9a-f]{64}$/.test(m.id)) throw new Error('モデルのID/版が不正です。');
  const f = m.features;
  ...
  if (m.schema_version === 2) {
    validateNormalModel(r.normal_model, f.dimension);
    if (!Number.isFinite(m.parity.calibrated_score_atol) || m.parity.calibrated_score_atol <= 0 || m.parity.calibrated_score_atol > .1) throw new Error('再尺度化スコアの照合設定が不正です。');
  } else if (r.normal_model) throw new Error('再尺度化モデルにはschema_version 2が必要です。');
  return m;
}
export function scoreVector(vector, ridge) {
  if (ridge.normal_model) return scoreNormalModel(vector, ridge.normal_model);
  return legacyScoreVector(vector, ridge);
}
function legacyScoreVector(vector, ridge) {
...
export function attribution(base, altered, scale = 0.6) {
  if (base.calibrated || altered.calibrated) {
    if (!base.calibrated || !altered.calibrated || base.calibration_id !== altered.calibration_id) throw new Error('異なる再尺度化モデルの差分は計算できません。');
    return finite(base.iq-altered.iq, '再尺度化後の除去差分');
  }
